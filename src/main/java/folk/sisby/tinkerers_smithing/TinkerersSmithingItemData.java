...
public record TinkerersSmithingItemData(Item item, Map<Ingredient, Integer> unitCosts, Set<Item> upgradePaths, Map<Item, Pair<Integer, Map<Item, Integer>>> sacrificePaths) {
	...
	public static TinkerersSmithingItemData read(PacketByteBuf buf) {
		...
		Set<Item> upgradePaths = buf.readCollection(HashSet::new, (buf2) -> Registries.ITEM.get(buf2.readIdentifier()));
		Map<Item, Pair<Integer, Map<Item, Integer>>> sacrificePaths = buf.readMap(
			(buf2) -> Registries.ITEM.get(buf2.readIdentifier()),
			(buf2) -> new Pair<>(
				buf2.readInt(),
				buf2.readMap((buf3) -> Registries.ITEM.get(buf3.readIdentifier()), PacketByteBuf::readInt)
			)
		);
	...
	public void write(PacketByteBuf buf) {
		...
		buf.writeCollection(upgradePaths, (buf2, item2) -> buf2.writeIdentifier(Registries.ITEM.getId(item2)));
		buf.writeMap(sacrificePaths, (buf2, item2) -> buf2.writeIdentifier(Registries.ITEM.getId(item2)), (buf2, pair) -> {
			buf2.writeInt(pair.getLeft());
			buf2.writeMap(pair.getRight(), (buf3, item2) -> buf3.writeIdentifier(Registries.ITEM.getId(item2)), PacketByteBuf::writeInt);
		});
	}
}
